/*
    ...
    5-stage MIPS pipeline main procedure
...
void printHelp()
{
    ...
    printf("-fwd [level]\t forwarding  level=0: no forwarding\n");
    printf("                           level=1: partial forwarding\n");
    printf("                           level=2: full forwarding\n");
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1;  i < argc;  i++) {
        if (!strcmp(argv[i], "-h")) {
            ...
            Pipe5Stage::forwardingLevel = atoi(argv[++i]);
        }
    }

    Pipe5Stage pipeline(iAssoc, iLogIndex, iLogBlockSize, dAssoc, dLogIndex, dLogBlockSize);
    if (interactiveMode)
